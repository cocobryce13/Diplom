...
static std::string build_page(const std::vector<std::string>& results, const std::string& query)
{
    ...
    ss << "<h3>Поиск</h3>";
    ss << "<form method='GET' action='/'><input name='q' value='" << query << "' size='60'/><input type='submit' value='Найти'/></form>";
    ss << "<hr/>";
    ss << "<div>Результаты (" << results.size() << ")</div><ul>";
    for (auto& r : results)
    {
...
int main()
{
    ...
    {
        std::cerr << "Ошибка setsockopt: " << WSAGetLastError() << std::endl;
